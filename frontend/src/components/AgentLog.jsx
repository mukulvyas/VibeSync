...
export default function AgentLog({ logs = [], fillContainer = false }) {
  ...
  }, [logs, visibleCount, fillContainer]);

  const renderedLogs = logs.length > 0 ? logs : STREAM_MESSAGES.slice(0, visibleCount);

  if (fillContainer) {
    return (
      <div className="rounded-2xl bg-[#1C2333]/30 border border-white/5 overflow-hidden">
        <div className="flex items-center gap-3 px-4 py-3 border-b border-white/5 bg-[#0A0E1A]/40 shrink-0">
        ...
        <div className="px-4 py-3 space-y-6 font-data">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="p-3 rounded-2xl rounded-tl-none bg-[#1C2333] border border-white/5 shadow-sm inline-block max-w-full">
                    <p className="text-[11px] leading-relaxed font-medium text-white/80">{log.text}</p>
                  </div>
                </div>
              </div>
            ))
